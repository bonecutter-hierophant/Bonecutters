...
function readDirtyPaths() {
  ...
  return unique(
    outputs
      ...
      .filter((value) => !value.startsWith(".tmp/verification-runner/"))
      .filter(Boolean)
  );
}
...
function printHelp() {
  console.log(`Usage:
  node tools/run-verification.mjs docs,client
  node tools/run-verification.mjs --gates docs,client
  node tools/run-verification.mjs preset:review
  node tools/run-verification.mjs --preset review
  node tools/run-verification.mjs --recommend --dirty
  node tools/run-verification.mjs --recommend paths:client/src/App.tsx
